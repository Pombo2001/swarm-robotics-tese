...
class ModernDashboardV6Pro(ctk.CTk):
    ...
    def update_metrics(self):
        ...
        if self.gnn_start_time: self.lbl_gnn_timer.configure(text=f"Tempo: {str(timedelta(seconds=int(time.time() - self.gnn_start_time)))}")
        if self.ppo_start_time: self.lbl_ppo_timer.configure(text=f"Tempo: {str(timedelta(seconds=int(time.time() - self.ppo_start_time)))}")
        if self.sac_start_time: self.lbl_sac_timer.configure(text=f"Tempo: {str(timedelta(seconds=int(time.time() - self.sac_start_time)))}")
    ...
    def load_current_config(self):
        try:
            ...
            scenario_map = {"none": "Nenhum (Modo Sandbox)", "u_wall": "Beco Sem Saída (Muro U)", "bottleneck": "Gargalo (Porta Estreita)", "four_rooms": "Quatro Salas (Labirinto)", "cooperative_door": "Porta Cooperativa (3 Robôs)"}
            self.combo_scenario.set(scenario_map.get(scenario, "Nenhum (Modo Sandbox)"))
        except Exception as e:
    ...
    def save_current_config(self):
        try:
            ...
            scenario_map_rev = {"Nenhum (Modo Sandbox)": "none", "Beco Sem Saída (Muro U)": "u_wall", "Gargalo (Porta Estreita)": "bottleneck", "Quatro Salas (Labirinto)": "four_rooms", "Porta Cooperativa (3 Robôs)": "cooperative_door"}
            config['environment']['classic_scenario'] = scenario_map_rev.get(combo_val, "none")

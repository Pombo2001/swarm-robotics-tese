...
class ModernDashboardV5(ctk.CTk):
    def __init__(self):
        super().__init__()

        self.title("🔬 Swarm Robotics Controller V5.1 - Ultimate Edition")
        self.geometry("1100x950")  # Aumentei um pouco a altura para caber tudo
        ...
        self.mode_selector = ctk.CTkSegmentedButton(self, values=["2D", "3D"], command=self.change_mode,
                                                    font=("Roboto", 16, "bold"), height=40, width=300)
        ...
        # --- PAINEL MESTRE DE TEMPO (SIMULTÂNEO) ---
        self.master_frame = ctk.CTkFrame(self, fg_color="#1e1e1e", border_width=2, border_color="#8E24AA")
        self.master_frame.pack(fill="x", padx=20, pady=10)

        ctk.CTkLabel(self.master_frame, text="⏱️ TREINO JUSTO SIMULTÂNEO (ORÇAMENTO DE TEMPO)",
                     font=("Roboto", 16, "bold"), text_color="#E0E0E0").pack(pady=5)

        time_input_frame = ctk.CTkFrame(self.master_frame, fg_color="transparent")
        time_input_frame.pack(pady=5)
        ctk.CTkLabel(time_input_frame, text="Tempo Máximo (Minutos):", font=("Roboto", 14)).pack(side="left", padx=10)
        self.entry_master_time = ctk.CTkEntry(time_input_frame, width=80, font=("Roboto", 14))
        self.entry_master_time.insert(0, "120")
        self.entry_master_time.pack(side="left")

        self.btn_start_both = ctk.CTkButton(self.master_frame, text="🚀 INICIAR GNN + PPO EM SIMULTÂNEO",
                                            command=self.start_both, fg_color="#D32F2F", hover_color="#B71C1C",
                                            font=("Roboto", 16, "bold"), height=40)
        self.btn_start_both.pack(pady=10, padx=20, fill="x")

        # --- MAIN FRAME (Para métricas e treinos individuais) ---
        ...
        # --- FOOTER ---
        self.btn_plot_thesis = ctk.CTkButton(self, text="📊 Gerar Gráficos Comparativos da Tese",
    ...
    def setup_gnn_ui(self):
        ...
        self.frame_gnn_title = ctk.CTkLabel(self.frame_gnn, text="🧬 Evolutivo (GNN)", font=("Roboto", 18, "bold"),
                                            text_color="#4CAF50")
        self.frame_gnn_title.pack(pady=10)

        # DE VOLTA: Caixa de input individual do GNN
        input_frame = ctk.CTkFrame(self.frame_gnn, fg_color="transparent")
        input_frame.pack(pady=5)
        self.lbl_gnn_target = ctk.CTkLabel(input_frame, text="Alvo (Gens):")
        ...
        self.lbl_gnn_best = ctk.CTkLabel(self.metrics_gnn, text="BEST FIT: ---", font=("Consolas", 16),
                                         text_color="#4CAF50")
        self.lbl_gnn_best.pack(pady=10)

        # DE VOLTA: Botão de Iniciar Treino Individual GNN
        self.btn_gnn_start = ctk.CTkButton(self.frame_gnn, text="▶ Iniciar Só GNN", command=self.start_gnn,
                                           fg_color="#4CAF50")
        self.btn_gnn_start.pack(pady=5, padx=20, fill="x")

        self.btn_gnn_stop = ctk.CTkButton(self.frame_gnn, text="⏹ Parar GNN", command=self.stop_gnn, fg_color="#F44336")
        ...
        self.btn_gnn_viz = ctk.CTkButton(self.frame_gnn, text="🎥 Visualizar Melhor", command=self.viz_gnn,
                                         fg_color="#1F6AA5")
        self.btn_gnn_viz.pack(pady=10, padx=20, fill="x")
    ...
    def setup_ppo_ui(self):
        ...
        self.frame_ppo_title = ctk.CTkLabel(self.frame_ppo, text="🤖 Por Reforço (PPO)", font=("Roboto", 18, "bold"),
                                            text_color="#FFA726")
        self.frame_ppo_title.pack(pady=10)

        # DE VOLTA: Caixa de input individual do PPO
        input_frame = ctk.CTkFrame(self.frame_ppo, fg_color="transparent")
        input_frame.pack(pady=5)
        self.lbl_ppo_target = ctk.CTkLabel(input_frame, text="Alvo (Steps):")
        ...
        self.lbl_ppo_score = ctk.CTkLabel(self.metrics_ppo, text="MEAN REW: ---", font=("Consolas", 16),
                                          text_color="#FFA726")
        self.lbl_ppo_score.pack(pady=10)

        # DE VOLTA: Botão de Iniciar Treino Individual PPO
        self.btn_ppo_start = ctk.CTkButton(self.frame_ppo, text="▶ Iniciar Só PPO", command=self.start_ppo,
                                           fg_color="#FFA726", text_color="black")
        self.btn_ppo_start.pack(pady=5, padx=20, fill="x")

        self.btn_ppo_stop = ctk.CTkButton(self.frame_ppo, text="⏹ Parar PPO", command=self.stop_ppo, fg_color="#F44336")
    ...
    def start_both(self):
        ...
        # O botão mestre força SEMPRE o uso de "--time_limit" em ambos
    ...
    def start_gnn(self):
        ...
        # Se 3D envia minutos, se 2D envia gerações
    ...
    def start_ppo(self):
        ...
        # Se 3D envia minutos, se 2D envia timesteps
    ...
    def update_metrics(self):
        ...
        if os.path.exists(paths["gnn_log"]):
            try:
                ...
                if not df.empty:
                    self.lbl_gnn_best.configure(text=f"BEST FIT: {df.iloc[-1]['best_fitness']:.1f}")
            except:
                pass
        ...
        if os.path.exists(paths["ppo_log"]):
            try:
                ...
                if not df.empty:
                    self.lbl_ppo_score.configure(text=f"MEAN REW: {df.iloc[-1]['ep_rew_mean']:.1f}")
            except:
                pass

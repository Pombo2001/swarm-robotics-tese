...
def plot_normalized_safe(ax, csv_path, label, color, y_col_name, smoothing_window):
    if os.path.exists(csv_path):
        ...
        if len(df) > 1 and y_col_name in df.columns:
            ...
            smoothed_series = df[y_col_name].rolling(smoothing_window, min_periods=1).mean()
...
def create_thesis_plots_3d():
    ...
    smoothing_window = plotting_config.get('smoothing_window', 10)
    ...
    def plot_individual(csv_file, y_col, title, color_light, color_dark, out_name):
        if os.path.exists(csv_file):
            ...
            if len(df) > 0 and y_col in df.columns:
                ...
                plt.plot(df[x_col], df[y_col].rolling(smoothing_window, min_periods=1).mean(), color=color_dark,
    ...
    plot_normalized_safe(ax1, gnn_csv, 'GNN 3D (Evolutivo)', '#2E7D32', 'best_fitness', smoothing_window)
    plot_normalized_safe(ax1, ppo_csv, 'PPO 3D (Actor-Critic)', '#E65100', 'ep_rew_mean', smoothing_window)
    plot_normalized_safe(ax1, sac_csv, 'SAC 3D (Soft Actor-Critic)', '#0277BD', 'ep_rew_mean', smoothing_window)

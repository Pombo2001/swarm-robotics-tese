...
for k in range(3, env.num_agents):
    env.agent_positions[k] = np.array([-12.0, -12.0, 0.0])
env.step(acoes)
porta_abriu = (not env.door_active) and np.all(env.walls[env.door_wall_index]['pos'] > 900)
if porta_abriu:
    print(f"  {OK} com 3 agentes a porta ABRE (door_active=False, parede removida)")
else:
    falhas += 1; print(f"  {FAIL} porta NÃO abriu com 3 agentes (door_active={env.door_active})")

...
\makeatletter
    ...
    \def\subsection{\@startsection{subsection}{2}%
      ...
      {\normalfont\bfseries\flushleft}}
\makeatother
% O \paragraph do amsbook é \normalfont: o título («Função de aptidão:», «Nível 1
% --- ...») saía com a letra do texto e confundia-se com ele. Mesmo espaçamento,
% mas a negrito, como as secções.
\makeatletter
...
Ao Professor Doutor Luís Nunes, meu orientador, pelo apoio e pela direção que me deu durante toda a jornada desta tese. Cada reunião ajudou-me a pôr ordem nas ideias, e cada revisão do texto trouxe perguntas que me fizeram voltar aos dados e confirmar o que afirmava antes de o escrever. Foi com essa exigência que o trabalho ganhou forma. Agradeço-lhe também a liberdade para seguir por caminhos que, à partida, nenhum de nós sabia onde iam dar.

Ao Iscte -- Instituto Universitário de Lisboa, pelas boas condições de trabalho ao longo do mestrado e, sobretudo, pelo empréstimo dos servidores em que treinei os algoritmos. Os resultados desta dissertação vêm de centenas de treinos, e fazê-los apenas no meu computador teria sido impossível.

À minha família, por estar sempre presente, do primeiro ao último dia deste percurso. Nunca me faltou uma palavra de apoio nem a paciência para os meses em que a tese ocupou quase todo o meu tempo. Aos meus pais, em particular, devo tudo o que me permitiu chegar até aqui.

À minha namorada, a quem este trabalho deve mais do que imagina. Acompanhou de perto cada fase, ouviu-me falar de robôs e de enxames mais vezes do que alguma vez quis, e aceitou sem queixas os fins de semana em que o computador levou a melhor. Nos dias em que nada corria bem, foi ela quem me deu ânimo para continuar e quem me lembrou que havia vida além da tese. Obrigado por nunca teres deixado de estar do meu lado.

À TAISCTE, por ter feito parte deste percurso de mestrado. As pessoas que lá conheci e os momentos que vivemos juntos deram a estes anos um equilíbrio de que precisei muito mais do que esperava.

Aos amigos e colegas que fiz pelo caminho, pelas conversas, pela entreajuda e pela boa companhia, e aos professores que deram o seu melhor para nos ensinar.
...
\label{sec:fund_mdp}
\paragraph{O Processo de Decisão de Markov:}
...
\begin{figure}[!htbp]
    ...
    \end{tikzpicture}
    \caption[Arquitetura do simulador e fluxo agente--ambiente]{Diagrama da arquitetura do simulador e fluxo de interação agente-ambiente.}
    \label{fig:sim_arch}
\end{figure}
...
\centering
\footnotesize
\caption[Hiperparâmetros da simulação e do treino]{Hiperparâmetros utilizados na simulação e no treino dos modelos.}
...
\begin{itemize}
    \item \texttt{src/} ($\approx$2\,700 linhas) --- o núcleo. Contém o ambiente (\path{environment/swarm_env_3d.py}: física, LiDAR, geodésicas, recompensa), os controladores (\texttt{agents/}: a rede \texttt{GNNAgent3D} e uma interface \texttt{Agent.get\_action}, hoje só usada pelo controlador aleatório de referência), os treinadores (\texttt{training/}: um módulo por paradigma) e o \emph{registo canónico de cenários} (\texttt{scenarios.py}).
    \item \texttt{scripts/} ($\approx$25\,600 linhas) --- a orquestração e a análise: \texttt{run\_experiments.py} (campanhas), a bateria de avaliação (\texttt{eval\_*.py}), os testes estatísticos, a geração de figuras e os verificadores que confrontam os números desta dissertação com os dados.
    \item \texttt{dashboard/} ($\approx$8\,300 linhas) --- observabilidade: acompanhamento do treino, curvas ao vivo, matriz de resultados e ligação ao servidor remoto.
    \item \texttt{tests/} ($\approx$4\,800 linhas) --- garantias de não-regressão, discutidas adiante.
\end{itemize}
...
A \emph{geometria}, porém, não goza da mesma disciplina: a construção de paredes, as posições de \textit{spawn} e as regras específicas de cada cenário estão distribuídas por mais de uma dúzia de blocos condicionais (\texttt{if classic\_scenario == \dots}) dentro do ambiente. Acrescentar um cenário implica, portanto, tocar em vários pontos de um ficheiro extenso --- um custo que cresce linearmente com o número de cenários e que constitui a dívida técnica mais visível deste projeto. A correção natural é um \emph{registo de cenários} análogo ao dos algoritmos: cada cenário passaria a ser uma classe com uma interface fixa (\texttt{build\_obstacles}, \texttt{spawn\_positions}, \texttt{required\_to\_eat}), registada por decorador, e esses condicionais colapsariam numa única invocação polimórfica. A refatoração é direta e está identificada; não foi executada porque as campanhas experimentais já tinham começado e qualquer alteração ao ambiente invalidaria a comparabilidade dos resultados já obtidos --- precisamente o tipo de compromisso entre pureza arquitetural e integridade experimental que uma dissertação empírica impõe.
...
\begin{figure}[!htbp]
    ...
    \caption[Curvas de aprendizagem no cenário Muro em U]{Curvas de aprendizagem no cenário Beco Sem Saída (Muro em U).}
...
\begin{figure}[!htbp]
    ...
    \caption[Curvas de aprendizagem no cenário Gargalo]{Curvas de aprendizagem no cenário Gargalo.}
...
\begin{figure}[!htbp]
    ...
    \caption[Curvas de aprendizagem no cenário Quatro Salas]{Curvas de aprendizagem no cenário Quatro Salas (Labirinto).}
...
\begin{figure}[!htbp]
    ...
    \caption[Curvas de aprendizagem no cenário Porta Cooperativa]{Curvas de aprendizagem no cenário Porta Cooperativa.}
...
\begin{figure}[!htbp]
    ...
    \caption[Curvas de aprendizagem no cenário Perceção Cooperativa]{Curvas de aprendizagem no cenário Perceção Cooperativa (Alvo Móvel).}
...
\begin{figure}[!htbp]
    ...
    \caption[Curvas de aprendizagem no cenário Porta com Alternativa]{Curvas de aprendizagem no cenário Porta Cooperativa com Alternativa (\textit{deceptive}).}
...
\begin{figure}[!htbp]
    ...
    \caption[Chegadas por execução: Sandbox e Muro em U]{Chegadas/ep por execução (7 execuções, 20 ep cada): Sandbox (esq.) e Muro em U (dir.). Cada ponto é uma execução independente e a barra vertical é a média reportada na tese; note-se que os dois painéis têm escalas horizontais diferentes.}
    \label{fig:res_box_1}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption[Chegadas por execução: Gargalo e Quatro Salas]{Chegadas/ep por execução: Gargalo (esq.) e Quatro Salas (dir.).}
    \label{fig:res_box_2}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption[Chegadas por execução: Porta Cooperativa e Perceção Cooperativa]{Chegadas/ep por execução: Porta Cooperativa (esq.) e Perceção Cooperativa (dir.).}
    \label{fig:res_box_3}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption[Chegadas por execução: Porta com Alternativa]{Chegadas/ep por execução: Porta Cooperativa com Alternativa (\textit{deceptive}).}
    \label{fig:res_box_4}
\end{figure}
...
O resultado evidencia uma resiliência elevada e \emph{transversal aos paradigmas}: a retenção de chegadas situa-se entre \emph{92\% e 106\%} em todas as 21 combinações algoritmo--cenário com desempenho de base, incluindo a Porta Cooperativa --- notável por exigir três agentes simultâneos para abrir a passagem: o enxame redistribui-se e mantém a função apesar da baixa. O controlador evolutivo retém 92--97\%. (Os valores pontualmente acima de 100\% correspondem a episódios em que a perda de agentes reduziu o congestionamento nas passagens estreitas.) A ausência de degradação catastrófica sugere que o \textit{parameter sharing} confere ao enxame uma redundância funcional implícita: nenhum agente é insubstituível, pelo que a falha parcial é absorvida pela reorganização do coletivo.
...
\centering
\footnotesize
\caption[Significância das diferenças entre algoritmos]{Significância das diferenças em chegadas por episódio (Mann-Whitney U sobre as médias por execução, $n=7$ por algoritmo; $\delta$ de Cliff; $\alpha = 0{,}05$).}
...
\caption[Configuração do ambiente de simulação e da física]{Configuração do ambiente de simulação e da física.}
\label{tab:apx_env}
...
\footnotesize
\renewcommand{\arraystretch}{1.15}
\caption[Hiperparâmetros de treino dos três algoritmos]{Hiperparâmetros de treino dos três algoritmos.}
\label{tab:apx_train}
\begin{tabular}{|l|l|c|}
\hline
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[
        cls/.style={rectangle split, rectangle split parts=2, draw=blue!70!black, fill=blue!4, thick,
                    text width=3.65cm, align=left, font=\scriptsize, inner sep=3pt, anchor=north},
...
\begin{lstlisting}[style=python, float=!htbp, caption={Proje\c{c}\~ao ortogonal do movimento contra a normal do muro.}]
...
\begin{lstlisting}[style=python, float=!htbp, caption={Campo de dist\^ancia geod\'esica ao ninho (Dijkstra sobre grelha).}]
...
\begin{lstlisting}[style=python, float=!htbp, caption={B\'onus de explora\c{c}\~ao por cobertura de c\'elulas novas.}]

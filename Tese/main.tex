...
% ── QI7, a declarar aqui quando a secção do mapa grande entrar (ver o bloco
% antes de \input{seccao_mapa_grande}). O texto é o mesmo da secção, e não
% depende do resultado: é a PERGUNTA, não a resposta.
%    \item[\textbf{QI7.}] \textbf{Composição de dificuldades:} As conclusões
%          obtidas em sete cenários de dificuldade isolada transferem-se para um
%          ambiente que \textbf{combina} essas dificuldades a uma escala
...
\begin{enumerate}
    ...
    %\begin{figure}[!htbp] \centering \includegraphics[width=0.6\textwidth]{images/scen_sandbox.png} \caption{Cenário Sandbox no visualizador.} \end{figure}
    ...
    %\begin{figure}[!htbp] \centering \includegraphics[width=0.6\textwidth]{images/scen_u_wall.png} \caption{Cenário Muro em U.} \end{figure}
    ...
    %\begin{figure}[!htbp] \centering \includegraphics[width=0.6\textwidth]{images/scen_bottleneck.png} \caption{Cenário Gargalo.} \end{figure}
    ...
    %\begin{figure}[!htbp] \centering \includegraphics[width=0.6\textwidth]{images/scen_four_rooms.png} \caption{Cenário Quatro Salas.} \end{figure}
    ...
    %\begin{figure}[!htbp] \centering \includegraphics[width=0.6\textwidth]{images/scen_cooperative_door.png} \caption{Cenário Porta Cooperativa.} \end{figure}
    ...
    %\begin{figure}[!htbp] \centering \includegraphics[width=0.6\textwidth]{images/scen_perception.png} \caption{Cenário Perceção Cooperativa.} \end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Orçamentos de treino por paradigma:} cada \textit{run} do controlador evolutivo dispôs de 195 minutos (população de 30 genomas avaliada em paralelo), e cada \textit{run} do PPO/SAC de 48 minutos (16 ambientes vetorizados). Os orçamentos foram calibrados, com base nas campanhas exploratórias, para levar cada paradigma ao seu planalto de convergência --- e, na maioria das células, é o que se observa. Uma verificação \textit{a posteriori} sobre as curvas (comparação emparelhada, por \textit{run}, entre a média do último quinto do orçamento e a do quinto anterior; \path{scripts/verificar_planalto.py}) mostra, porém, que \textbf{7 das 21 combinações ainda subiam de forma significativa no fim} --- e duas delas de forma marcada: o SAC no Gargalo ($+112\%$) e na Porta com Alternativa ($+105\%$), seguidos do GNN no Muro em U ($+33\%$) e do PPO no Quatro Salas ($+27\%$). A comparação mede, portanto, o \textbf{desempenho atingível dentro do orçamento concedido}, e não o desempenho assintótico de cada paradigma nem a eficiência amostral a cómputo igual; nas quatro células assinaladas, os valores do algoritmo em causa devem ser lidos como \emph{limite inferior}. A implicação mais forte é para o SAC nos gargalos físicos: a fragilidade que a Secção~\ref{sec:res_boxplots} lhe atribui é indistinguível, com os dados retidos, de sub-treino --- os históricos da campanha final guardaram a recompensa episódica mas não a métrica de tarefa por passo, pelo que não é possível decidir se o crescimento final corresponde a mais recolhas ou apenas a mais \textit{shaping}. A assimetria de cómputo (favorável ao evolutivo) é retomada na discussão (Secção~\ref{sec:res_discussao}).
...
\begin{figure}[!htbp]
    ...
    \caption[Taxa de sucesso ($P_{task}$) por cenário e algoritmo]{Taxa de sucesso ($P_{task}$) por cenário, em avaliação determinística (7 \textit{runs} $\times$ 20 episódios emparelhados por algoritmo).}
...
\begin{figure}[!htbp]
    ...
    \caption[Recolhas por episódio, por cenário e algoritmo]{Recolhas por episódio, por cenário. Barra = média sobre os 140 episódios de avaliação (7 \textit{runs} $\times$ 20); barra de erro = desvio padrão \emph{entre episódios}, que nos cenários de convergência bimodal (Muro U, Sandbox-GNN, Gargalo-SAC) chega a exceder a própria média --- a dispersão que lhe está por trás é a dispersão entre \textit{runs} da Secção~\ref{sec:res_boxplots}. Mede a magnitude do desempenho, para além do sucesso binário.}
...
\begin{figure}[!htbp]
    ...
    \caption[Potencial de navegação no Muro U: euclidiano vs.\ geodésico]{Potencial de navegação no Muro U: euclidiano (esquerda) vs.\ geodésico (direita). As curvas de nível indicam a direção para a qual o termo de progresso atrai o agente; só o geodésico contorna a barra do U.}
...
\begin{figure}[!htbp]
    ...
    \caption[Densidade de ocupação dos robôs no Muro U (GNN, PPO e SAC)]{Densidade de ocupação dos robôs no Muro U (GNN, PPO e SAC, da esquerda para a direita; 6 episódios por painel, escala logarítmica). O GNN contorna a barra do U pela esquerda e o PPO pela direita --- em ambos, um fluxo contínuo até ao ninho, com 473 e 412 recolhas. O SAC não resolve o cenário: a ocupação concentra-se \emph{colada} às faces interiores das paredes verticais, o padrão de agentes presos, e o episódio termina com \textbf{0 recolhas}. A escala de cor é normalizada por painel, pelo que as cores comparam a distribuição \emph{dentro} de cada algoritmo, não a magnitude entre eles.}
...
\begin{figure}[!htbp]
    ...
    \caption[Curvas de aprendizagem no cenário Sandbox]{Curvas de aprendizagem dos três algoritmos no cenário Sandbox. O eixo das abcissas está normalizado (0--100\% do orçamento de treino); linha = média de 7 \textit{runs}, banda = $\pm 1$ desvio padrão.}
...
\section{Fiabilidade e Variância entre \textit{Runs}}
\label{sec:res_boxplots}
A análise da fiabilidade de treino requer múltiplas execuções independentes por configuração, de modo a distinguir o desempenho sistemático do ruído de inicialização. As Figuras~\ref{fig:res_box_1}--\ref{fig:res_box_4} mostram, para cada cenário, \textbf{um ponto por \textit{run}} --- as recolhas por episódio desse \textit{run} (média dos 20 episódios de avaliação; 7 pontos por algoritmo) --- com a média reportada na tese marcada a traço vertical. A métrica é de tarefa, na mesma unidade para os três algoritmos, ao contrário dos \textit{scores} de treino. Com $n=7$ mostram-se os \textit{runs} um a um em vez de um diagrama de caixa: nestes dados a distribuição é frequentemente \emph{bimodal} --- \textit{runs} que resolvem o cenário e \textit{runs} que ficam a zero, sem nada pelo meio --- e uma caixa a cobrir todo esse intervalo sugeriria densidade onde não existe uma única execução.

A leitura por cenário quantifica o que as secções anteriores anteciparam. Nos cenários \textbf{cooperativos} (Porta Cooperativa, Porta com Alternativa) e no \textbf{Quatro Salas}, os sete \textit{runs} dos três algoritmos ficam agrupados e afastados do zero --- treinar é \emph{fiável} --- e o GNN posiciona-se sistematicamente acima do SAC (e, no Quatro Salas, muito acima de ambos os métodos de gradiente). No \textbf{Gargalo}, PPO e GNN são fiáveis (7/7 \textit{runs}), mas os \textit{runs} do SAC espalham-se de 0 a 88 recolhas/ep --- o treino é uma lotaria. No \textbf{Sandbox} inverte-se: PPO/SAC agrupados, GNN com dois \textit{runs} degenerados. No \textbf{Muro U}, os três algoritmos têm \textit{runs} a zero: nenhum garante a convergência, e só o PPO tem a maioria das execuções funcionais (4/7). Esta análise por \textit{run} é a base dos testes de significância da Secção~\ref{sec:res_discussao}.

\begin{figure}[!htbp]
    \centering
    \figresultado{0.49}{images/resultados/dotplot_eval_none.png}{Fiabilidade entre runs --- Sandbox}
    \hfill
    \figresultado{0.49}{images/resultados/dotplot_eval_u_wall.png}{Fiabilidade entre runs --- Muro U}
...
\begin{figure}[!htbp]
    \centering
    \figresultado{0.49}{images/resultados/dotplot_eval_bottleneck.png}{Fiabilidade entre runs --- Gargalo}
    \hfill
    \figresultado{0.49}{images/resultados/dotplot_eval_four_rooms.png}{Fiabilidade entre runs --- Quatro Salas}
...
\begin{figure}[!htbp]
    \centering
    \figresultado{0.49}{images/resultados/dotplot_eval_cooperative_door.png}{Fiabilidade entre runs --- Porta Cooperativa}
    \hfill
    \figresultado{0.49}{images/resultados/dotplot_eval_cooperative_perception.png}{Fiabilidade entre runs --- Perceção Cooperativa}
...
\begin{figure}[!htbp]
    \centering
    \figresultado{0.49}{images/resultados/dotplot_eval_cooperative_door_bypass.png}{Fiabilidade entre runs --- Porta Cooperativa com Alternativa}
...
\begin{figure}[!htbp]
    ...
    \caption[Desempenho do GNN (evolutivo) nos sete cenários]{Desempenho do GNN (evolutivo) nos sete cenários (linha = média entre os 7 \textit{runs} por progresso de treino).}
...
\begin{figure}[!htbp]
    ...
    \caption[Desempenho do PPO nos sete cenários]{Desempenho do PPO nos sete cenários (linha = média entre os 7 \textit{runs} por progresso de treino).}
...
\begin{figure}[!htbp]
    ...
    \caption[Desempenho do SAC nos sete cenários]{Desempenho do SAC nos sete cenários (linha = média entre os 7 \textit{runs} por progresso de treino).}
...
\begin{figure}[!htbp]
    ...
    \caption[Trajetórias: contorno do Muro U e navegação no Quatro Salas]{Trajetórias completas de um episódio do controlador evolutivo (20 agentes; cor = instante do episódio): contorno do obstáculo em U (esq., 78 recolhas) e navegação no labirinto de Quatro Salas (dir., 72 recolhas). Nenhuma trajetória atravessa uma parede: os saltos de reaparecimento após entrega no ninho estão excluídos do traçado.}
...
\paragraph{Replicação com poder estatístico: o mega-treino a $n=28$.} Os quatro testes acima assentam em $7$ execuções por braço --- suficientes para a inferência sobre médias, mas curtas onde a leitura depende de \emph{contagens}: com $n=7$, um teste exato de Fisher sobre a convergência não é reportável, e o $\delta = +0{,}59$ de (T4) ficou à espera de poder estatístico. Uma campanha final de um mês de máquina, com \textbf{pré-registo próprio} fixado antes de existirem dados de avaliação, repetiu por isso o cenário crítico com \textbf{quatro vezes} as execuções. As células a $n=28$ são \textbf{autocontidas} --- execuções novas, que o pré-registo proíbe expressamente somar às sete anteriores --- e cobrem os quatro braços no Muro em U (GNN adaptativo, GNN objetivo, PPO e SAC), todos com o orçamento por execução da campanha final.

O resultado principal (M1) confirma-se e reforça-se: o adaptativo faz $67{,}4 \pm 13{,}4$ recolhas/ep contra $32{,}5 \pm 32{,}5$ do objetivo puro ($p < 0{,}0001$ unilateral, $\delta = +0{,}61$) e, sobretudo, a convergência --- agora reportável --- separa os braços sem ambiguidade: \textbf{$28/28$ execuções a 100\% de sucesso contra $15/28$} (Fisher exato, $p < 0{,}0001$). É a \emph{única} condição de toda a dissertação em que todas as execuções resolvem o Muro em U. A comparação com os métodos de gradiente ao mesmo $n$ (M2) coloca o adaptativo acima de ambos --- PPO $35{,}6 \pm 36{,}0$ ($14/28$; $p = 0{,}0010$, $\delta = +0{,}51$) e SAC $10{,}1 \pm 14{,}2$ ($14/28$; $p < 0{,}0001$, $\delta = +0{,}99$) ---, ao passo que o GNN \emph{objetivo} e o PPO permanecem indistinguíveis ($p = 0{,}088$): a vantagem não é da neuroevolução enquanto tal, mas da \textbf{dosagem adaptativa da exploração}. A Figura~\ref{fig:megatreino_uwall} mostra porque é que a média sozinha engana neste cenário --- os três braços sem anilamento repartem-se entre execuções que resolvem e execuções que ficam a zero, praticamente sem nada pelo meio, enquanto o adaptativo não tem uma única execução falhada. Assinala-se que os seis pares de M2 são $p$ brutos: o pré-registo manda declarar a multiplicidade e ancorar a leitura no tamanho de efeito, não corrigir os valores.
...
\begin{figure}[!htbp]
    ...
    \caption[Eficiência por agente com o aumento do enxame (Sandbox), sem retreino]{Eficiência \emph{por agente} (recolhas por episódio a dividir pelo número de agentes) com o aumento do enxame, sem retreino, no cenário Sandbox. Só a linha do controlador de grafo atravessa toda a gama: o PPO e o SAC têm um único ponto, em $N=20$, porque a política MLP de entrada fixa não admite outras dimensões de enxame.}
...
\begin{figure}[!htbp]
    ...
    \caption[Resiliência à perda súbita de 10\% dos agentes ($R_{robust}$)]{Resiliência do enxame perante a perda súbita de 10\% dos agentes a meio do episódio ($R_{robust}$). Os três painéis partilham a escala vertical, pelo que as alturas são comparáveis entre algoritmos; o rótulo é a percentagem de recolhas retida face à avaliação sem falhas, nas mesmas \textit{seeds}. Os $106\%$ do SAC na Perceção Cooperativa não são um erro de medida: nesse cenário o alvo é móvel e um enxame ligeiramente menor congestiona-o menos --- a diferença ($16{,}5 \rightarrow 17{,}5$ recolhas/ep) está dentro do desvio entre episódios.}
...
\paragraph{Custo computacional e eficiência.} A competitividade do evolutivo na tarefa tem um preço de cómputo: cada \textit{run} do GNN consumiu 195 minutos com $\approx 30$ núcleos (avaliação paralela da população), contra 48 minutos com 16 núcleos do PPO/SAC --- uma razão de $\approx 8\times$ em núcleos-hora. Esta assimetria é também o limite da comparação: como a verificação da Secção~\ref{sec:res_metodologia} mostra, o orçamento do PPO/SAC não foi suficiente para atingir o planalto em quatro células --- três delas do SAC ---, pelo que a leitura correta é a de \emph{desempenho atingível dentro do orçamento concedido}, com os valores dessas células a funcionarem como limite inferior. A conclusão que resiste é a inversa da que o cómputo sugeriria: mesmo com $8\times$ menos núcleos-hora, os métodos de gradiente igualam ou superam o evolutivo em quatro dos sete cenários --- em \emph{eficiência amostral e computacional} a sua vantagem é clara, e deve pesar na escolha de paradigma quando o orçamento de treino é o recurso escasso.
...
% ── Parágrafo da QI7 nas Conclusões — escolher UM e apagar os outros dois ────
% Entra aqui, antes do parágrafo de fecho, quando a secção do mapa grande entrar.
%
% (A) ≥15/21 num algoritmo:
...
% de $103 \times 62$\,m ---, acrescenta a estas conclusões uma distinção que os
% cenários isolados não permitiam fazer. Nenhum dos vinte e um campeões entrega
% um único item neste mapa sem retreino, e as três causas alternativas ao efeito
% da topologia foram excluídas por condições de controlo; mas
% \PORPREENCHER{algoritmo}, treinado de raiz, resolve-o em \PORPREENCHER{k} das
% $21$ execuções. O que a composição quebra é a transferência, não a
% aprendibilidade.
%
% (B) 0 execuções convergentes:
% Um oitavo cenário, construído para \emph{compor} as dificuldades que os sete
% isolam --- gargalo, beco em U, quatro salas e porta cooperativa, num labirinto
% de $103 \times 62$\,m ---, delimita o alcance destas conclusões. Nenhum dos
% vinte e um campeões entrega um único item neste mapa sem retreino, e nenhum dos
% três algoritmos o resolve com treino nativo dentro do orçamento pré-registado,
% embora o mapa seja demonstravelmente resolúvel (um oráculo geodésico recolhe
% $53$ itens por episódio). O resultado é negativo e é reportado como tal: as
% conclusões desta dissertação valem para dificuldades isoladas, e a sua
% composição a esta escala está fora do alcance dos três métodos como aqui
% treinados.
%
% (C) entre 1 e 14 convergentes:
% Um oitavo cenário, construído para \emph{compor} as dificuldades que os sete
% isolam --- gargalo, beco em U, quatro salas e porta cooperativa, num labirinto
% de $103 \times 62$\,m ---, mostra onde estas conclusões se degradam primeiro.
% Nenhum dos vinte e um campeões entrega um único item sem retreino; com treino
% nativo, \PORPREENCHER{algoritmo} resolve o mapa em \PORPREENCHER{k} das $21$
% execuções, abaixo do limiar fixado antes dos dados. O que a composição degrada
% não é a magnitude do que se consegue, mas a \emph{fiabilidade} com que se
% consegue --- a mesma bimodalidade que o Muro em U já expunha, agora num mapa
...
% ── RESPOSTA À QI7 — escolher UMA e apagar as outras duas ────────────────────
% ⚠️ Fica DENTRO do `description` de propósito: descomentado a seguir ao
% \end{description}, o \item ficava órfão e o LaTeX dava «Lonely \item». O ensaio
% (scripts/testes/ensaio_integracao_qi7.py) apanhou-o hoje; a 16 de agosto teria
% custado o tempo que não há.
% As três leituras seguem a regra pré-comprometida (⌈5/7 × 21⌉ = 15 execuções
% convergentes em pelo menos um algoritmo, emendas 19 e 21 do pré-registo).
%
% (A) quinze ou mais execuções convergentes num algoritmo:
%    \item[\textbf{QI7 --- Composição de dificuldades.}] Sim, com retreino: o
%          \PORPREENCHER{algoritmo} resolve o mapa composto em
%          \PORPREENCHER{k}/$21$ execuções, com \PORPREENCHER{valor} recolhas por
%          episódio. O contraste com o zero-shot é a resposta: as políticas
%          treinadas nos sete cenários isolados não entregam um único item neste
%          mapa (84 células a $0{,}00$), mas o mesmo algoritmo, treinado de raiz,
%          resolve-o --- o que falha na composição é a \emph{transferência}, não
%          a aprendibilidade (Secção~\ref{sec:res_mapa_grande}).
%
% (B) nenhuma execução convergente --- resultado negativo honesto:
%    \item[\textbf{QI7 --- Composição de dificuldades.}] Não, dentro do orçamento
%          pré-registado: nenhum dos três controladores resolve o mapa composto,
%          nem por transferência (84 células a $0{,}00$) nem com treino nativo.
%          Três verificações independentes excluem que a causa seja o desenho do
%          mapa --- existe caminho para todos os agentes, um oráculo geodésico
%          recolhe $53$ itens por episódio, e a folga do orçamento é de
%          $2{,}9\times$ ---, pelo que o resultado delimita o alcance dos três
%          métodos sob composição e escala. Um mapa que nenhum algoritmo resolve
%          é um resultado, não uma campanha falhada
%          (Secção~\ref{sec:res_mapa_grande}).
%
% (C) entre uma e catorze --- negativo pela regra, com o número declarado:
%    \item[\textbf{QI7 --- Composição de dificuldades.}] Parcialmente, e ao preço
%          da fiabilidade: o mapa composto é resolúvel por
%          \PORPREENCHER{algoritmo}, mas só em \PORPREENCHER{k} das $21$ execuções
%          --- abaixo do limiar de $15$ fixado antes dos dados, pelo que a QI7
%          \textbf{não sobe a resposta afirmativa}. O que a composição degrada é a
%          \emph{variabilidade entre execuções}, não a magnitude: é a assinatura
...
% ── A QI7 (mapa grande) entra AQUI, se o F2 fechar antes de 22 ago ───────────
% A secção está escrita e é a última coisa a entrar na tese; descomentar a linha
% do \input é tudo o que é preciso do lado do LaTeX. Ensaiado a 6 ago com a
% secção incluída: compila em 129 páginas (+8), 0 referências indefinidas, 0
% overfull e 0 floats grandes demais — o que falta a 16 ago é preencher os cinco
% \PORPREENCHER e escolher UMA das três leituras, não descobrir problemas.
%
% Os outros TRÊS sítios onde a QI7 tem de aparecer estão todos escritos em
% comentário, nos três desfechos, para que nesse dia seja escolher e descomentar:
%   1. a PERGUNTA, na lista de questões (Secção~\ref{sec:questoes_investigacao});
%   2. a RESPOSTA, no fim da lista acima (aqui em baixo);
%   3. uma frase no Resumo e outra no Abstract.
%
% Se o F2 NÃO fechar a tempo, fica tudo como está — a regra 4 dos compromissos de
% reporte do pré-registo manda-o para a defesa, e a tese está completa sem ele.
%
...
\begin{enumerate}
    \item \textbf{Alargamento da bateria estatística onde ainda é $n=7$:} a replicação a $28$ execuções por braço fechou a questão no cenário que mais a exigia --- o Muro em U, onde a leitura depende de contagens e não de médias (Secção~\ref{sec:res_novelty}) ---, mas o alargamento foi \emph{assimétrico} e deixa três frentes por cobrir. Primeiro, nas células exploratórias do Sandbox e da Perceção Cooperativa só o braço \emph{adaptativo} correu a $21$ execuções, mantendo-se os comparadores a $7$; é essa assimetria de dimensões amostrais, e não a direção do efeito, que mantém o $\delta = +0{,}39$ do Sandbox longe do limiar convencional, pelo que alargar o braço de controlo é o que falta para o decidir. Segundo, as $21$ execuções do SAC no Gargalo correram com o orçamento reduzido de $48$ minutos fixado no pré-registo para essa célula, não sendo por isso comparáveis com a campanha final: o $5/7$ dessa combinação continua por replicar ao orçamento de $195$ minutos. Terceiro, as vinte e uma combinações da campanha final permanecem a $7$ execuções --- dimensão suficiente para a inferência sobre médias, mas curta para contrastar taxas de convergência, que é a leitura que os cenários bimodais exigem.

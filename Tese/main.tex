...
\begin{enumerate}
    ...
    \item \textbf{Chen et al. (2024)} --- demonstra a eficácia das \textbf{GNNs} na coordenação descentralizada com transferibilidade a dimensões de enxame não vistas \cite{sun2024graph}, validando a arquitetura de grafo com atenção adotada no controlador evolutivo desta tese.
    \item \textbf{Zhang et al. (2022)} --- GNNs hierárquicas para exploração multi-robô, operando sobre vizinhanças de dimensão variável \cite{zhang2022h2gnn}; confirma que a invariância à dimensão do enxame é uma propriedade da \emph{representação}, e não do otimizador (QI2).
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{2.6cm}|p{1.9cm}|p{4.4cm}|p{4.4cm}|}
        ...
        Chen et al. (2024) \cite{sun2024graph} & MARL (GNN) & Transferibilidade \textit{Zero-Shot} a dimensões de enxame não vistas via grafos com atenção. & Minimização de erro de estimação; sem obstáculos físicos complexos nem \textit{baseline} evolutiva. \\ \hline

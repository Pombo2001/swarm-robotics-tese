...
Mais próximo da formulação adotada nesta dissertação, Yigit et al. (2026) propõem um sistema de navegação de enxames totalmente descentralizado, modelado como um \textit{Multi-Agent Markov Decision Process} (MAMDP) e treinado exclusivamente com \textbf{PPO} e \textit{parameter sharing}, sem comunicação explícita entre agentes \cite{yigit2026marl}. Os autores reportam convergência rápida e taxas de sucesso quase perfeitas numa arena 2D com obstáculos circulares estáticos, recorrendo a uma função de recompensa multicomponente (progresso, penalização de colisões, \textit{team bonus}) da mesma família da aqui empregue, o que valida empiricamente o paradigma PPO descentralizado adotado como \textit{baseline}. Contudo, este trabalho evidencia precisamente as lacunas que a presente tese pretende preencher: o espaço de observação é de dimensão fixa, impossibilitando o \textit{Zero-Shot Transfer} para enxames de tamanho variável; a avaliação restringe-se à comparação contra uma política aleatória, sem testes estatísticos nem cenários de \textit{stress}; e os próprios autores remetem para trabalho futuro a inclusão de obstáculos dinâmicos, agentes heterogéneos e falhas de comunicação --- aspetos contemplados na metodologia desta dissertação.
...
\begin{enumerate}
    ...
    \item \textbf{Yigit et al. (2026)} --- PPO descentralizado com \textit{parameter sharing} em navegação de enxame \cite{yigit2026marl}, da mesma família da \textit{baseline} aqui adotada; a sua avaliação contra política aleatória, sem \textit{Zero-Shot} nem testes de falha, ilustra o défice de rigor comparativo que motiva este trabalho.
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{2.6cm}|p{1.9cm}|p{4.4cm}|p{4.4cm}|}
        ...
        Yigit et al. (2026) \cite{yigit2026marl} & MARL (PPO) & PPO descentralizado com \textit{parameter sharing} em navegação de enxame. & Observação de dimensão fixa (sem \textit{Zero-Shot}); compara-se apenas com política aleatória. \\ \hline
    \end{tabular}
\end{table}

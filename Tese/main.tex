...
\usepackage{chngcntr}
\usepackage{graphicx}
\usepackage{pdfpages} % formulário de uso de IA, no Anexo A
...
\begin{itemize}
    \item \textbf{Desempenho da Tarefa:} Avaliar a eficiência absoluta na concretização de objetivos globais (ex: taxa de cobertura de área ou número de chegadas cooperativas a um ponto de encontro). Seguindo a linha de Majid et al. (2024), pretende-se quantificar se a convergência mais lenta do MARL resulta, de facto, numa política final superior em eficiência energética comparativamente aos métodos bio-inspirados \cite{majid2023deep}.
...
\begin{enumerate}
    \item \textbf{Arena Aberta (Sandbox):} Cenário base sem barreiras fixas. Contém os agentes, obstáculos dinâmicos e um ninho móvel. Serve para aferir a capacidade pura de cooperação e gestão de energia sem entropia espacial acrescida.
...
\begin{itemize}
    ...
    \item \textbf{Novidade:} $\nu(\theta_k)$ é a distância euclidiana média de $\mathbf{b}(\theta_k)$ aos seus $k_{nn}=10$ vizinhos mais próximos no conjunto formado pela população atual e por um \emph{arquivo} de comportamentos passados (os 3 mais novos de cada geração; capacidade máxima de 1000, gerida em regime FIFO).
...
Na \textbf{redação do documento}, as ferramentas foram usadas para revisão linguística e estilística, sugestão de reformulações, verificação da coerência terminológica entre secções e apoio à formatação em \LaTeX. Não foram usadas para produzir conteúdo científico: a formulação das questões de investigação, a revisão sistemática da literatura e a triagem dos estudos, a leitura e interpretação dos resultados experimentais, a discussão e as conclusões são trabalho do autor. Nenhuma referência bibliográfica foi incluída sem verificação direta na fonte original.
...
O autor assume a inteira responsabilidade pelo conteúdo desta dissertação, incluindo pelas partes em que houve apoio das ferramentas aqui declaradas. Nas páginas seguintes reproduz-se o formulário do Iscte sobre o uso de ferramentas de inteligência artificial, preenchido pelo autor.

\includepdf[pages=-,pagecommand={}]{anexos/formulario_uso_ia.pdf}

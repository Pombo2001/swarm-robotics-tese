...
\begin{figure}[!htbp]
    \centering
    \figresultado{0.95}{images/resultados/heatmap_geodesico_u_wall.png}{Potencial euclidiano vs geodesico no Muro em U}
...
\begin{figure}[!htbp]
    \centering
    \figresultado{0.32}{images/resultados/heatmap_ocupacao_gnn_u_wall.png}{Ocupacao GNN no Muro em U}
    \hfill
    \figresultado{0.32}{images/resultados/heatmap_ocupacao_ppo_u_wall.png}{Ocupacao PPO no Muro em U}
    \hfill
    \figresultado{0.32}{images/resultados/heatmap_ocupacao_sac_u_wall.png}{Ocupacao SAC no Muro em U}
    \caption[Densidade de ocupação dos robôs no Muro em U (GNN, PPO e SAC)]{Densidade de ocupação dos robôs no Muro em U (GNN, PPO e SAC, da esquerda para a direita; 6 episódios por painel, escala logarítmica). O GNN contorna a barra do U pela esquerda e o PPO pela direita --- em ambos, um fluxo contínuo até ao ninho, com 473 e 412 recolhas. O SAC não resolve o cenário: a ocupação concentra-se \emph{colada} às faces interiores das paredes verticais, o padrão de agentes presos, e o episódio termina com \textbf{0 recolhas}. A escala de cor é normalizada por painel, pelo que as cores comparam a distribuição \emph{dentro} de cada algoritmo, não a magnitude entre eles.}
...
A leitura por cenário quantifica o que as secções anteriores anteciparam. Nos cenários \textbf{cooperativos} (Porta Cooperativa, Porta com Alternativa) e no \textbf{Quatro Salas}, os sete \textit{runs} dos três algoritmos ficam agrupados e afastados do zero --- treinar é \emph{fiável} --- e o GNN posiciona-se sistematicamente acima do SAC (e, no Quatro Salas, muito acima de ambos os métodos de gradiente). No \textbf{Gargalo}, PPO e GNN são fiáveis (7/7 \textit{runs}), mas os \textit{runs} do SAC espalham-se de 0 a 88 recolhas/ep --- o treino é uma lotaria. No \textbf{Sandbox} inverte-se: PPO/SAC agrupados, GNN com dois \textit{runs} degenerados. No \textbf{Muro em U}, os três algoritmos têm \textit{runs} a zero: nenhum garante a convergência, e só o PPO tem a maioria das execuções funcionais (4/7). Esta análise por \textit{run} é a base dos testes de significância da Secção~\ref{sec:res_discussao}.

\begin{figure}[!htbp]
    \centering
    \figresultado{0.49}{images/resultados/dotplot_eval_none.png}{Fiabilidade entre runs --- Sandbox}
    \hfill
    \figresultado{0.49}{images/resultados/dotplot_eval_u_wall.png}{Fiabilidade entre runs --- Muro em U}
    \caption{Recolhas/ep por \textit{run} (7 \textit{runs}, 20 ep cada): Sandbox (esq.) e Muro em U (dir.).}
    \label{fig:res_box_1}
\end{figure}

...
\chapter*{Resumo}
Para comparar a Aprendizagem por Reforço Multiagente e a Otimização Bio-inspirada no controlo de enxames robóticos, implementou-se num simulador de alta fidelidade o \textit{framework} descentralizado \textit{Robust and Scalable Swarm Control} (partilha de parâmetros; PPO e SAC) e um controlador neuroevolutivo sobre grafos com atenção, invariante à dimensão do enxame. Avaliaram-se em sete cenários de dificuldade crescente (7 execuções independentes por combinação), sob estatística não-paramétrica.

Com \textit{homing} geodésico na função de \textit{fitness}, o controlador evolutivo supera os métodos de gradiente em três cenários, iguala o melhor deles noutros dois e transfere, sozinho, sem retreino para dimensões não vistas ($N$ de $10$ a $100$, com 100\% de sucesso); os de gradiente mantêm a fiabilidade em espaço aberto e a eficiência. O cenário de deceção espacial, bimodal nos três algoritmos base, só cedeu à novidade doseada \emph{adaptativamente}: preserva os $7/7$ execuções no Muro em U e atinge, na Porta com Alternativa, o melhor resultado de toda a dissertação ($88{,}7$ recolhas/ep); uma replicação pré-registada com $28$ execuções por braço confirmou-o ($28/28$, contra $15/28$ do objetivo puro, $14/28$ do PPO e $14/28$ do SAC). Um oitavo cenário, compondo num labirinto de $103 \times 62$\,m as dificuldades dos sete, degrada a fiabilidade e não a magnitude: é resolvido em 4 das $21$ execuções independentes, abaixo do limiar pré-registado.

A hipótese de que a inteligência adaptativa supera a robustez estática confirma-se apenas em parte: a escala decide-se na \emph{representação} e não no otimizador; sob gradientes enganadores, no sinal de treino.
...
\chapter*{Abstract}
To compare Multi-Agent Reinforcement Learning and Bio-inspired Optimization for the control of robotic swarms, we implemented in a high-fidelity simulator the decentralized framework \emph{Robust and Scalable Swarm Control} (parameter sharing; PPO and SAC) and a neuroevolutionary controller over graphs with attention, invariant to swarm size. Both were evaluated across seven scenarios of increasing difficulty (7 independent runs per combination), under non-parametric statistics.

With geodesic \emph{homing} shaping in the fitness function, the evolutionary controller outperforms the gradient-based methods in three scenarios, matches the best of them in two others, and alone transfers without retraining to unseen swarm sizes ($N$ from $10$ to $100$, at 100\% success); the gradient-based ones retain reliability in open space and efficiency. The spatial-deception scenario, bimodal across the three base algorithms, only yielded to novelty dosed \emph{adaptively}: it preserves the $7/7$ runs on the U-shaped wall and attains, on the Alternative-Path Door, the best result of the entire dissertation ($88.7$ collections/ep); a pre-registered replication with $28$ runs per arm confirmed this ($28/28$, against $15/28$ for the pure objective, $14/28$ for PPO and $14/28$ for SAC). An eighth scenario, composing in a $103 \times 62$\,m maze the difficulties of the seven, degrades reliability rather than magnitude: it is solved in 4 of $21$ independent runs, below the pre-registered threshold.

The hypothesis that adaptive intelligence surpasses static robustness is thus only partly confirmed: scale is decided by the \emph{representation} and not by the optimizer; under deceptive gradients, by the training signal.

...
\begin{enumerate}
    ...
    \item \textbf{O mapa composto, com o orçamento separado em três causas:} a QI7 fecha com $4$ das $21$ execuções a resolver o mapa (Secção~\ref{sec:res_mapa_grande}), e a instrumentação por distância geodésica permite dizer \emph{o que} falta --- que não é uma causa só. Primeira: o \textbf{orçamento de treino} não chegou ao planalto. Em $19$ das $21$ execuções o melhor \textit{fitness} surge nos últimos $20\%$ das gerações, com a geração do máximo a cair, em mediana, a $95\%$ do orçamento; nas quatro execuções que resolvem o mapa, a descoberta traduz-se num salto de \textit{fitness} de mais de uma ordem de grandeza nessa fase final. Alargar os $780$ minutos por execução é, portanto, a via com mais indicação empírica, e a que o \emph{schedule} da novidade (item anterior) mais beneficiaria. Segunda: o \textbf{horizonte do episódio} limita a conclusão da tarefa, mas não a descoberta do percurso. Duplicar \texttt{max\_steps} de $2000$ para $4000$ \emph{sem qualquer retreino} leva de $3$ para $5$ o número de execuções com pelo menos uma recolha, e duplica a magnitude da melhor delas (de $11$ para $22$ recolhas por episódio); a distância mínima mediana ao ninho, porém, não se altera --- o horizonte destrava quem já chegava perto, não quem ficou pelo caminho. Terceira, e distinta das duas: quatro execuções param a $5$--$13$\,m do ninho e não entram nele mesmo com o dobro do tempo, o que aponta para a fase final de aproximação --- e não para o orçamento --- como o alvo do próximo refinamento do sinal de treino ou da observação. As três medições estão em \path{results/mapa_grande/} e usam um episódio determinístico por execução, contra os $20$ da avaliação oficial: são diagnóstico para orientar trabalho, não substituem o protocolo de avaliação.

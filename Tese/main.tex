...
\chapter*{Abstract}
To compare Multi-Agent Reinforcement Learning and Bio-inspired Optimization for the control of robotic swarms, we implemented in a high-fidelity simulator the decentralized framework \emph{Robust and Scalable Swarm Control} (parameter sharing; \emph{Proximal Policy Optimization} and \emph{Soft Actor-Critic}) and a neuroevolutionary controller over graphs with attention, invariant to swarm size. Both were evaluated across seven scenarios of increasing difficulty (7 independent runs per combination), under non-parametric statistics.

With geodesic \emph{homing} shaping in the fitness function, the evolutionary controller outperforms the gradient-based methods in three scenarios, matches the best of them in two others, and alone transfers without retraining to unseen swarm sizes ($N$ from $10$ to $100$, at 100\% success); the gradient-based ones retain reliability in open space and efficiency. The spatial-deception scenario, bimodal across the three base algorithms, only yielded to novelty dosed \emph{adaptively}: it preserves the $7/7$ runs on the U-shaped wall and attains, on the Alternative-Path Door, the best result of the entire dissertation ($88.7$ collections per episode); a pre-registered replication with $28$ runs per arm confirmed this ($28/28$, against $15/28$ for the pure objective and $14/28$ for each gradient-based method). An eighth scenario, composing in a $103 \times 62$\,m maze the difficulties of the seven, degrades reliability rather than magnitude: it is solved in 4 of $21$ independent runs, below the pre-registered threshold.
...
\begin{figure}[!htbp]
    ...
    \caption[Densidade de ocupação dos robôs no Muro em U (GNN, PPO e SAC)]{Densidade de ocupação dos robôs no Muro em U (GNN e PPO em cima, SAC em baixo; 6 episódios por painel, escala logarítmica). O GNN contorna a barra do U pela esquerda e o PPO pela direita --- em ambos, um fluxo contínuo até ao ninho, com 476 e 419 recolhas. O SAC não resolve o cenário: a ocupação concentra-se \emph{colada} às faces interiores das paredes verticais, o padrão de agentes presos, e o episódio termina com \emph{0 recolhas}. A escala de cor é normalizada por painel, pelo que as cores comparam a distribuição \emph{dentro} de cada algoritmo, não a magnitude entre eles.}
...
\begin{figure}[!htbp]
    ...
    \caption{Recolhas/ep por execução (7 execuções, 20 ep cada): Sandbox (esq.) e Muro em U (dir.). Cada ponto é uma execução independente e a barra vertical é a média reportada na tese; note-se que os dois painéis têm escalas horizontais diferentes.}
    \label{fig:res_box_1}
\end{figure}

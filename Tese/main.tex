...
\begin{itemize}
    \item \textbf{Desempenho da Tarefa:} Avaliar a eficiência absoluta na concretização de objetivos globais (ex: taxa de cobertura de área ou número de chegadas cooperativas a um ponto de encontro). Pretende-se quantificar se a convergência mais lenta do MARL resulta, de facto, numa política final superior em eficiência energética comparativamente aos métodos bio-inspirados.
...
Ao estabelecer estas definições, este capítulo permite identificar o ``Deployment Gap'' (fosso de implantação) discutido por Kegeleirs e Birattari (2025), justificando a necessidade de uma metodologia rigorosa para validar a transição destes algoritmos do domínio da simulação para aplicações reais de alta fidelidade \cite{kegeleirs2025towards}.

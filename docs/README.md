...
Cada um fixa hipótese, testes, regra de decisão e compromissos de reporte antes
da campanha correr. **Emendas são datadas dentro do próprio ficheiro, nunca
silenciosas** — é isso que os torna pré-registos e não relatórios.
...
## Propostas e registos históricos

Ficam aqui de propósito: mostram **porque** se decidiu o que se decidiu, e várias
frases da tese apoiam-se neles. Não são para atualizar.
...
Nada aqui deve ser atualizado nem citado como estado atual.

| Ficheiro | Porque saiu de circulação |
|---|---|
| `PLANO_DE_ATAQUE.md` | Registo histórico até 19 jul; fundido no `PLANO_MESTRE.md`. |
| `PLANO_ATAQUE_FINAL.md` | Lista acionável de 15 jul; fundida no `PLANO_MESTRE.md`. |
| `tese_snapshot_19jun2026.pdf` | Snapshot da tese de 19 jun que estava em `Tese/` ao lado do `main.pdf` verdadeiro, com nome de ficheiro oficial — era o PDF que se anexa a um e-mail por engano. O PDF a usar é sempre `Tese/main.pdf`, recompilado. |

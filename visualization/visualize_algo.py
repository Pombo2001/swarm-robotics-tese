"""O visualizador 3D ao vivo, um só, para os três algoritmos.
...
Substitui os antigos `visualize_gnn.py`, `visualize_ppo.py` e `visualize_sac.py`,
que diferiam em seis sítios (nome do algoritmo, título da janela, pasta do
modelo, prefixo do ficheiro, classe que carrega e linha que imprime). Tudo o que
varia está no dicionário `ALGOS`.

O desenho exagera a escala do que é pequeno de mais para se ver (altura e
espessura das paredes, tamanho dos robôs) e declara todos esses exageros no HUD.
As POSIÇÕES nunca são exageradas: são as do simulador.

Opções: barra de velocidade, teto de passos por frame, telemetria de proximidade
que colore os robôs, `--scenario`/`--agents` em memória (sem escrever no
`configs/foraging.yaml`) e `--models-root` para espreitar campanhas arquivadas.
"""
import argparse
import math
import os
...
# Paleta. A cor do robô é a MESMA que o algoritmo tem nas figuras da tese
# (`src/scenarios.ALGO_COLORS`), para se reconhecer o braço sem legenda; os
# cinzentos vêm do tema do dashboard, pela mesma razão.
def _hex(s):
    s = s.lstrip('#')
    return color.rgb32(int(s[0:2], 16), int(s[2:4], 16), int(s[4:6], 16))


# Os tons têm de SEPARAR-SE, não só combinar: com fundo e paredes a tons
# próximos, uma parede só se vê do lado que a luz apanha. Há três degraus claros
# entre fundo, chão e parede, e as arestas são mais claras do que ambos.
...
# Raiz dos modelos: por omissão a pasta ATIVA (results/), mas pode apontar para
# os modelos arquivados de uma campanha (results/graficos_tese/<sessão>/modelos),
# que têm a mesma estrutura — evita restaurar modelos por cima dos ativos.
_ap.add_argument('--models-root', type=str, default=None)
# Rastos e grafo: DESLIGADOS por omissão. Com 20 agentes, ligar cada um aos
# vizinhos e às posições anteriores dá centenas de segmentos cruzados sobre a
# mesma área e tapa a cena; ficam disponíveis para ver a topologia ou o caminho
...
# Captura: corre N passos sem intervenção, grava um PNG e fecha.
...
# reescreve o configs/foraging.yaml.
if _args.scenario:
...
# Iluminação a três pontos: principal (define a forma), preenchimento (abre as
# sombras, senão o lado escuro fica preto e os robôs somem contra o fundo) e
# rasante (separa os contornos do chão).
_luz = DirectionalLight(y=3, z=-2, x=1, rotation=(50, -30, 0),
                        color=color.rgb32(255, 250, 240))   # principal, quente
# Sombras projetadas: sem elas os objetos flutuam sobre o chão e a vista lê-se
# como um diagrama. Só a luz PRINCIPAL as projeta e a caixa de sombra é limitada
# à arena, o que as torna baratas; `--sem-sombras` desliga-as.
if _args.sombras:
    _luz.shadows = True
    try:
...
# Vista inicial ISOMÉTRICA, não de topo (que achata tudo e faz o 3D parecer um
# mapa 2D). Com o campo de visão: câmara muito recuada dá projeção quase
# ortográfica, sem fuga, e o resultado volta a parecer um diagrama — aproxima-se
# a câmara e alarga-se o fov.
#
# Ângulo por omissão consoante a arena, medido em capturas. NESTA CONVENÇÃO,
# MENOS GRAUS É MAIS DE CIMA: nos sete cenários (raio 15) 52° dá volume sem tapar
# nada; num labirinto de 103 m com muros de 6 m o mesmo ângulo põe os muros à
# frente dos corredores, e a 40° vê-se o labirinto por dentro.
if _args.angulo is None:
    _args.angulo = 40.0 if env.arena_radius > 30.0 else 52.0
_camera_editor.rotation_x = _args.angulo
# O plano do MAPA (x,y do simulador) é vertical no espaço do Ursina, porque a
# cena é desenhada 1:1 com o mundo: `rotation_x=90` dá a vista de topo e baixar o
# ângulo inclina. Ao inclinar, o centro da arena sobe no ecrã e sobra uma faixa
# preta em baixo — desloca-se o pivô ao longo de Y para recentrar.
...
# Chão contínuo, que se estende para lá da arena e morre na névoa: um disco
# recortado no preto lê-se como um objeto a flutuar, e os anéis concêntricos de
# uma grelha métrica desenham um alvo que puxa o olho para o centro geométrico,
# que não é onde a ação está. A fronteira real da arena fica marcada por um anel
# de luz no chão e outro à altura de 2,5 m.
R = env.arena_radius
...
def _linhas(v, thickness=1):
    ...
    `Mesh(vertices=v, mode='line')` sozinho desenha uma linha CONTÍNUA: liga
...
def _anel(raio, z, cor, alpha=1.0, thickness=1, lados=96):
    ...
    Era `Entity(model='circle', mode='line')` — e essa combinação NÃO faz
...
def _esfera_arame(raio, cor, alpha=0.16, meridianos=8, paralelos=5, lados=72):
    ...
    Sem isto a cena mente por omissão. A arena não é um recinto plano: é uma
...
if _args.chao == 'plano':
    # Translúcido: o «cima» do ecrã é -z, logo um agente com z > 0 está do OUTRO
    # lado deste plano e, opaco, desaparecia do ecrã (no mapa grande, onde o teto
    # é ±2 m, isso escondia metade do enxame). O plano é uma referência, não um
    # chão.
    ...
    # sem escrita de profundidade, para nunca apagar um agente.
    ...
    # (não uma entidade por linha) — são ~90 segmentos. Acaba ANTES do plano
    # (2,2 R contra 3,4 R): levada até ao fim, as linhas distantes chegam quase
    # rasantes ao ecrã e emaranham-se num feixe no canto.
...
# Ninho: o ninho é uma ZONA (raio 1,5 m) e não um objeto, por isso desenha-se
# como alvo no chão com uma cúpula baixa, e não como esfera com halo — que no
# ecrã competia com o enxame inteiro.
...
# que diz de onde está pendurado.
...
# Sombra de contacto dos obstáculos: a arena é uma ESFERA e os obstáculos
# ocupam-na em altura, pelo que sem a marca no chão os que estão a 10 m
# projetam-se para fora do anel da arena e parecem pedras a flutuar.
...
# Paredes: opacas, com aresta, e à ALTURA VISUAL. As paredes reais têm 2x o raio
# da arena (30 m para raio 15) — é o que as torna estanques —, mas desenhadas a
# essa escala transformam o mapa num poço, e a única vista possível passa a ser a
# de topo. Desenham-se a `ALTURA_VISUAL` (um sexto do raio, mínimo 2 m), que
# permite inclinar a câmara; o HUD diz sempre a altura REAL, para o ecrã não
# afirmar uma geometria que não é a do simulador.
ALTURA_REAL = float(env.walls[0]['size'][2]) if len(env.walls) else 0.0
# A altura visual é medida contra a passagem mais estreita: uma parede muito mais
# alta do que a passagem é larga transforma a vista num telhado — vê-se a caixa
# por fora e não o percurso lá dentro.
...
# Engrossamento VISUAL das paredes, pela mesma lógica da altura: exagera-se a
# espessura do desenho, nunca a posição. No mapa grande as paredes têm 1,5 m para
# 103 m de mapa e ao natural são riscos. O alvo é 1/25 do raio da arena, cortado
# a um quarto da passagem mais apertada — a passagem tem de continuar visivelmente
# aberta, senão o desenho mente ao contrário.
...
# E a altura pela mesma medida: `R/4` dava 15 m no mapa grande contra passagens
# de 2,4 m, e o que aparecia eram os TOPOS das paredes, com os robôs escondidos
# nos corredores. Limitar a 2,5x a passagem mantém a leitura de labirinto.
if _PASSAGEM:
...
for i, w in enumerate(env.walls):
    ...
    # Engrossa-se SÓ a dimensão menor (a espessura): aumentar o comprimento
    # estenderia a parede para dentro da abertura que está na sua ponta.
    ...
    # `model='cube', mode='line'` NÃO desenha arame: desenha um cubo cheio
    # cinzento por cima da parede, que a deixa chapada e esconde a cor da porta.
    # As doze arestas têm de ser construídas à mão.
    wall_edges.append(Entity(model=_linhas(_arestas_caixa(escala), 2),
                             color=GRELHA, alpha=0.55, unlit=True,
                             position=pos))
    # A parede REAL vai até `ALTURA_REAL` (2x o raio da arena). Sólida, faz da
    # cena um poço; ausente, a cena parece 2D. Fica em arame: vê-se até onde veda
    # e vê-se através.
...
# Névoa: o que está longe esbate-se na cor do fundo. Sem isso, uma parede a 5 m e
# outra a 40 m têm o mesmo tom e a cena lê-se como um plano.
try:
...
# Robôs: corpo na cor que o algoritmo tem nas figuras da tese, cúpula clara que
# dá volume e visor que mostra a direção.
def _aviva(c, f=1.75):
...
# Escala VISUAL dos robôs: ao tamanho físico (raio 0,15 m numa arena de raio 15)
# um robô ocupa um centésimo do diâmetro e não se distingue do fundo. Desenham-se
# 3,5x maiores e o HUD diz o raio real. O que NÃO se exagera é a posição.
ESCALA_ROBO = 3.0
# Numa arena de raio 15 o robô a 3x já se vê; no mapa grande (raio 60) 0,45 m em
# 103 m é meio pixel. O tamanho no ecrã tem um mínimo ligado ao raio da arena.
...
for r_pos in env.agent_positions:
    ...
    # Sombra de contacto: disco escuro colado ao chão por baixo de cada robô. É o
    # que o assenta na superfície — sem ela, os robôs parecem flutuar.
...
# Marcador vertical: haste fina na posição (x, y) exata do robô, que sobe acima
# das paredes. DESLIGADO por omissão — o efeito é o de um raio-X, com robôs a
# aparecerem por cima de paredes que na vista escolhida os deviam tapar, e um
# visualizador que mostra através da geometria não serve para julgar se o enxame
# contorna ou atravessa. Fica em `--pinos` para localizar o enxame de propósito.
...
# O GRAFO: as ligações que a política vê. A arquitetura é uma rede de grafo com
# atenção sobre os vizinhos, e desenhá-lo mostra a topologia a densificar-se
# quando o enxame se junta no gargalo e a rarefazer-se quando se dispersa.
#
# Desenha-se em UM só `Mesh` de linhas, reconstruído por frame: 20 agentes dão
# até 190 pares, e uma entidade por aresta seria insustentável.
...
def _atualizar_grafo():
    ...
    # Sem os pares em `triangles`, o Mesh liga o fim de cada aresta ao início da
    # seguinte, e o que aparece não é o grafo dos vizinhos mas um caminho a
    # atravessar a arena de lado a lado.
    grafo.model.triangles = [(k, k + 1) for k in range(0, len(v), 2)]
    grafo.model.generate()


# Rastos: um enxame não se lê pela pose instantânea, mas pelo caminho que
# desenha — o funil à entrada do gargalo, o beco onde alguns ficam presos, a fila
# na porta. Janela curta de posições, desenhada como linhas esbatidas num só Mesh.
PASSOS_RASTO = 26
historico = []
...
# HUD: com três janelas abertas lado a lado — que é como se comparam — é preciso
# dizer no ecrã qual é o algoritmo, o cenário e quantas recolhas vão feitas.
...
# O HUD declara TODOS os exageros do desenho, que é o que os torna aceitáveis. E
# diz também que nos cenários com paredes os agentes nascem todos em z = 0: quem
# olha e acha que «parece 2D» está a ver bem, no primeiro passo é.
...
def _atualizar_cena():
    ...
    # O anel acompanha o ninho: no Sandbox ele MUDA de sítio a cada recolha
    # (`_spawn_nest`) e o anel ficava para trás, aparecendo dois ninhos no ecrã.
    # Segue as TRÊS coordenadas — no Sandbox o ninho pode estar a 8 m de altura —
    # e a marca no chão fica por conta da sombra.
    ...
    # As PAREDES têm de ser atualizadas a cada frame: quando a porta cooperativa
    # abre, o ambiente REMOVE-A de `env.walls`, e é este ciclo que a faz
    # desaparecer do ecrã.
    for i, wall in enumerate(env.walls):
        if i < len(wall_views):
            # O z tem de continuar a ser o VISUAL (-ALTURA_VISUAL/2, que assenta
            # a caixa no chão): copiar `wall['pos']` inteiro punha a parede
            # centrada em z=0, metade enterrada.
    ...
    for i, r_pos in enumerate(env.agent_positions):
        ...
        # A telemetria de proximidade fica só para o SINAL e para o contacto com
        # paredes: com o enxame agrupado no gargalo ficava tudo ciano («perto de
        # outro robô») e a cor do algoritmo desaparecia.
...
def _capturar_e_sair():
    ...
    # «não se veem robôs na imagem» é comportamento (amontoados num canto) ou
    # desenho (espalhados, e a imagem não os mostra).
